...
def getElmoEmbeddingMatrix(wordIndex, dim):
    ...
    with io.open(os.path.join(gloveDir, 'elmo.csv'), 'rb') as f:
        for line in f:
            line = line.decode('utf8')
            values = line.split('\t')
            word = values[0]
            vec = values[1][1:len(values[1]) - 3]
            vec = vec.split(',')
...
def main():
    ...
    print('K-fold start')
    print('-'*60)
    for clf, name in [cnnModel(gloveEmbeddingMatrix, embedding_dim=200, hidden_dim=120, name='gloveCnn'),
                      attentionModel(gloveEmbeddingMatrix, embedding_dim=200, hidden_dim=120, name='gloveAttention'),
                      ...
                      cnnModel(elmoEmbeddingMatrix, embedding_dim=1024, hidden_dim=400, name='elmoCnn'),

...
def preprocessData(dataFilePath, mode):
    ...
    with io.open(dataFilePath, encoding="utf8") as finput:
        ...
        for line in finput:
            ...
            for index in range(len(list_str)):
                if list_str[index] != '<eos>' and len(list_str[index]) > 2:
                    list_str[index] = sp.correct(list_str[index].lower())
    ...
    df.to_csv('./eos.csv', sep='\t', index=False)
    if mode == "train":
...
def buildModel(embeddingMatrix):
    ...
    embedded = Embedding(input_dim=embeddingMatrix.shape[0], output_dim=EMBEDDING_DIM, input_length=MAX_SEQUENCE_LENGTH, weights=[embeddingMatrix],
                         trainable=False)(sequence)
    embedded = Dropout(0.25)(embedded)
    # convolutional layer
    convolution = Convolution1D(filters=60,
                                kernel_size=3,
                                padding='valid',
                                activation='relu',
                                strides=1
    ...
    dense = Dense(128)(maxpooling)  # best: 120
    dense = Dropout(0.15)(dense)  # best: 0.25

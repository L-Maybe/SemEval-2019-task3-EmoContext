...
from keras.layers import Dense, Dropout, Embedding, CuDNNGRU, Bidirectional, GRU, Input, Flatten, SpatialDropout1D, Concatenate
...
def main():
    ...
    lexicon = train_lexicon[trainIndices]
    ...
    for k in range(NUM_FOLDS):
        ...
        xtrain_lexicon = np.vstack((lexicon[:index1], lexicon[index2:]))
        yTrain = np.vstack((labels[:index1], labels[index2:]))
        xVal = data[index1:index2]
        xVal_lexicon = lexicon[index1:index2]
        yVal = labels[index1:index2]
        print("Building model...")
        model = capsulnetModel(embeddingMatrix)
        early_stopping = EarlyStopping(monitor='val_acc', patience=5)
        model.fit([xTrain, xtrain_lexicon], yTrain,
                  validation_data=([xVal, xVal_lexicon], yVal),
                  epochs=NUM_EPOCHS, batch_size=BATCH_SIZE, verbose=2, callbacks=[early_stopping])
        path = './model/{0}'.format('capsulnet_model')
        if not os.path.exists(path):
            os.makedirs(path)
        model.save('./model/%s/bi-%s-model-fold-%d.h5' % ('capsulnet_model', 'capsulnet_model', k))

        predictions = model.predict([testData, trial_lexicon], batch_size=BATCH_SIZE)
        metrics['result'].append(predictions)
        
    data_processed = os.path.join('pickle', 'k-fold-prediction.pickle')
    pickle.dump([metrics], open(data_processed, 'wb'))

    for item in metrics['result']:
        predictions += item

    predictions = predictions.argmax(axis=1)
    pickle.dump(predictions, open('./pickle/elmo.pickle', 'wb'))
